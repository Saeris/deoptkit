...
How to put real application and library code under deoptkit's observation, and where the tool hooks into everyday JS/TS development workflows. This extends [SPEC.md](./SPEC.md) §8; that section's summary defers to this document.

## 1. The two kinds of signal, and why it changes the strategy

deoptkit emits two classes of signal with opposite statistical properties:
...
Traditional benchmarking lives entirely in the timing class and is notoriously un-CI-able. deoptkit's practical edge is the structural class: **"did this change make `parseRecord` megamorphic?" is a stable, machine-independent question — gateable in CI exactly like a failing test.** The strategy that follows uses structural signals as the regression gate and timing signals only for ranking (which finding matters most) and local investigation.
...
## 3. Harness helpers: `deoptkit/harness` (v1.1)
...
## 4. Vitest integration: `deoptkit/vitest` (v1.1)
...
- `deoptkit/serve` helper: `driveServer({ start, requests, warmups })` — boots an app's HTTP handler in-process, fires `warmups + requests` fetches at it, then exits cleanly. One process, full observation, works with anything exposing a Node server.
...
## 7. CI regression gating: `deoptkit ci` (v1.2)

The payoff of the structural/timing split — a snapshot-test workflow for optimization health:

```bash
deoptkit ci bench/parse.bench.mjs            # run + compare against baseline
deoptkit ci --update bench/parse.bench.mjs   # accept current findings as baseline
```
...
- Runs the same bin, no MCP host needed: the existing `main.ts` grows argv dispatch (`deoptkit` with no args = stdio server, unchanged).
...
| v1.2    | `deoptkit ci` + baselines (with GitHub `::warning` annotations), `deoptkit/serve` + Next/Astro/build recipes                                                                        | Makes it durable (CI) and applicable to apps, not just libraries |
| v1.3    | `deoptkit lsp` inline diagnostics (the vitest-bench perf-linting loop), child-process flag propagation research, `get_profile` call trees, npm publish of the above                 | Depth and reach                                                  |
